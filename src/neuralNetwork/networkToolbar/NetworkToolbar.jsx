...
const Item = (props) => {
  ...
  return (
    <span style={style}>
      {props.label}:{" "}
      <strong style={{ color: "rgb(10,10,10)" }}>{props.value}</strong>
    </span>
  );
};
...
export default class extends React.Component {
  constructor(props) {
    ...
    this.snapshotSlider = React.createRef();
  }
  _onSliderChange(event, value) {
    this.props.dispatch(setSnapshotIndex(value));
  }
  render() {
    ...
    return (
      ...
      >
        ...
        >
          <strong>
            Epoch {snapshotIndex} of {this.props.totalEpochs}
          </strong>
          - Scroll through "learning phases"
        </div>
        <div style={{ width: "400px", display: "inline-block" }}>
          <Slider
            ref={this.snapshotSlider}
            style={{ marginBottom: "20px" }}
            step={1}
            primary
            value={snapshotIndex}
            max={this.props.totalEpochs}
            onChangeCommitted={this._onSliderChange}
          />
        </div>
      </div>
    );
