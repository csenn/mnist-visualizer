...
import { calculateSigmoid } from "../networkUtils";
...
class LayerModal extends React.Component {
  ...
  _renderActivationCalculation(layerIndex, nodeIndex) {
    if (!this.props.selectedDrawing) {
      return (
        <div style={{ marginTop: "20px", color: "rgb(140, 140, 140)" }}>
    ...
    let str =
      "a_{j}^{l} =\\sigma(" + roundedSummation + " + " + roundedBias + ")";
    const activation = this.props.selectedNetworkSummary.activation;
    str +=
      "=" +
      Math.round(calculateSigmoid(activation, bias + summation) * 1000) / 1000;
  ...
  _renderCalulationHeader(layerIndex, nodeIndex) {
    const all = (
      <span>
        ...
        <span
          style={{ display: "inline-block", width: "5px", textAlign: "center" }}
        >
          <LatexRenderer value="*" />
        </span>
        <span
        ...
        >
          <LatexRenderer
            value={"w_{" + (nodeIndex + 1) + "k}^{" + (layerIndex + 2) + "}"}
          />
        </span>
      </span>
    );
  ...
  _renderCalculations(layerIndex, nodeIndex) {
    ...
    const weightViews = weights.map((weight, index) => {
      ...
      if (this.props.selectedDrawing) {
        x =
          Math.round(
            this.props.selectedDrawing.activations[layerIndex][index][0] * 1000,
          ) / 1000;
      ...
      const all = (
        <span>
          ...
          <span style={getStyle("5px", "rgb(140, 140, 140)")}>*</span>
          <span style={getStyle()}>{Math.round(weight * 1000) / 1000}</span>
        </span>
      );
  ...
  render() {
    ...
    const actions = [<Button label="Done" secondary onClick={this._onClose} />];
