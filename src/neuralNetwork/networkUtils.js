...
export function feedDrawingThroughNetwork(
  x,
  biases,
  weights,
  ...
  const activations = [x];
  const zs = [];
  const boundCalculcateSigmoid = _.partial(
    calculateSigmoid,
    selectedNetworkSummary.activation,
  );
...
export function convertToGraph(selectedSnapshot, dataPoint) {
  ...
  biases.forEach((biasLayer, layerIndex) => {
    ...
    biasLayer.forEach((bias, biasIndex) => {
      ...
      if (activations) {
        node.activation = roundThousandth(
          activations[layerIndex + 1][biasIndex],
        );
      }
