...
export function backProp(x, y, biases, weights) {
  ...
  const derivativeZ = applyFunctionOverMatrix(
    zs[zs.length - 1],
    calculateSigmoidPrime,
  );
  ...
  const tranposeSecondLastLayer = transposeMatrix(
...
export function feedForward(a, biases, weights) {
  for (let i = 0; i < biases.length; i++) {
    a = applyFunctionOverMatrix(
      addMatrix(crossProduct(weights[i], a), biases[i]),
      calculateSigmoid,
    );
  }
  return a;
...
export function runEpochs(options, initialBiases, initialWeights) {
  const { trainingData, epochs, miniBatchSize, eta, testData, onStateUpdate } =
    options;

  for (let i = 0; i < epochs; i++) {
    const miniBatches = splitIntoMiniBatches(
      _.shuffle(trainingData),
      miniBatchSize,
    );
    ...
    miniBatches.forEach((batch) => {
      const { weights, biases } = updateMiniBatch(
        batch,
        eta,
        curBiases,
        curWeights,
    ...
    if (testData) {
      ...
      console.log(
        `Epoch ${i + 1} complete: ${count} out of ${testResults.length}`,
      );
    } else {
