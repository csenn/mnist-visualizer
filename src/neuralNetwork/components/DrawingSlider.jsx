...
function isInRange(index, nextIndex) {
  ...
  if (index < nextIndex) {
    return (
      Math.abs(index - nextIndex) < 4 || Math.abs(30 + index - nextIndex) < 4
    );
  }
  return (
    Math.abs(nextIndex - index) < 4 || Math.abs(30 + nextIndex - index) < 4
  );
}
  ...
  render() {
    return (
      <div style={{ display: "inline-block", width: "500px" }}>
        ...
        >
          <FormControlLabel
            control={<Switch onChange={this._onToggle} />}
            sx={{ fontFamily: "Raleway", fontSize: "26px" }}
